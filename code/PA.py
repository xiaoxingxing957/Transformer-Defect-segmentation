...
def calculate_accuracy(binary_image, reference_image):
    # 确保输入图像是二值化图像
    ...
    # 计算相同像素数
    correct_pixels = np.sum(binary_image == reference_image)
...
# 计算准确率
accuracy = calculate_accuracy(binary_image, reference_image)

print(f'二值化图像的像素准确率为: {accuracy:.2%}')